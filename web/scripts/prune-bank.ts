...
const ALGO_RULES: Rule[] = [
  ...
  { name: "демо-апплеты учебника", re: /Workshop|апплет|applet|кнопк[аиуе]\b|щёлк|щелк|нажатии кнопки/i, scope: "text" },
  { name: "про книгу и автора", re: /\bавтор\b|в книге|в главе|данной книг|этой книг|для читателей/i, scope: "text" },
 ...
 */
const DB_RULES: Rule[] = [
  { name: "интерфейс инструментов", re: /кнопк[аиуе]\b|щёлк|щелк|Query Analyzer|Enterprise Manager|диалогов(ое|ом) окн|вкладк|мастер[а]? настройки|какая утилита|какое приложение/i, scope: "text" },
  { name: "администрирование вне спецификации", re: /резервн(ое|ого) копирован|\bbackup\b|восстановлени[ея] базы|роли пользовател|права доступа|учётн(ая|ые) запис|аутентификац|администратор баз|кто обычно занимается/i, scope: "text" },
  { name: "не из раздела 4", re: /триггер|хранимы[ех] процедур|\bкурсор[ыа]?\b|\bNoSQL\b|репликац|в каком году|истори[яи] развития/i, scope: "text" },
  { name: "про книгу и автора", re: /\bавтор\b|в книге|в главе|данной книг|этой книг|для читателей/i, scope: "text" },
];

const RULES: Record<string, Rule[]> = { algo: ALGO_RULES, db: DB_RULES };

/** Задания со скриншотов реального пробника не трогаем: они по определению
 *  в программе теста, даже если формулировка выходит за раздел 4. */
